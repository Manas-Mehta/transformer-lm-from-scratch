...
def create_augmented_dataloader(args, dataset):
    ...
    sampled_indices = random.sample(range(len(train_split)), 5000)
    sampled_subset = train_split.select(sampled_indices)
    transformed_subset = sampled_subset.map(custom_transform, load_from_cache_file=False)

    augmented = datasets.concatenate_datasets([train_split, transformed_subset])
    augmented = augmented.map(tokenize_function, batched=True, load_from_cache_file=False)
    augmented = augmented.remove_columns(["text"])
    augmented = augmented.rename_column("label", "labels")
    augmented.set_format("torch")
